...
- **수동으로 시리즈 묶기**: 자동 인식이 놓친 시리즈(제목에 권수 표기가 없거나 표기 방식이 다른 경우)를 위해, 화면 좌하단 "수동 묶기"를 켜면 각 표지 좌상단에 선택 체크박스가 나타납니다. 2권 이상 선택 후 "시리즈로 묶기"를 누르고 이름을 입력하면 그 책들이 하나의 덱으로 묶입니다. 자동 인식 시리즈와 동일하게 배지/펼치기/전체 선택을 지원합니다. 펼친 헤더에서 **"이름 변경"**으로 이름을 바꾸고, **"책 추가"**로 수동 묶기 선택 모드에 들어가 더 담을 책을 고른 뒤 "추가"로 반영할 수 있으며, 각 권 표지 우상단의 **✕**로 그 권만 묶음에서 뺄 수 있습니다(2권 미만이 되면 묶음이 자동으로 해제됩니다). "그룹 삭제"는 묶음 전체를 없앱니다 — 어떤 경우든 책 자체는 삭제되지 않습니다.
...
- 수동 묶기 그룹의 책 추가/제거는 volume id 기준으로 동작하므로, 같은 책이 서재에서 완전히 삭제되면 다음 렌더링 때 그 그룹에서도 자연히 빠집니다.
